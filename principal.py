...
def PontoColeta(lat, lon, mat):
    ...
    res[2] = 1000000000000 
    for i in range(len(ct.material)):
        dist = abs(math.sqrt((ct.latitude[i] - lat)**2 + (ct.longitude[i] - lon)**2 ))
  ...
  if opcao_menu_principal == 1:
    ...
    while (opcao_menu_principal != 6):
        ...
        if opcao_menu_info == 1:
          ...
          print("A reciclagem do Papel é um processo que visa utilizar  papéis velhos com o aproveitamento das fibras celulósicas dos papéis e cartões já utilizados, para produção de papéis novos.")
        elif opcao_menu_info == 3:
          print("A reciclagem de Metal consiste no reaproveitamento do aço contido em objetos obsoletos, sendo usado em vários materiais, como carros, latas, suportes de concreto, tendo sua reciclagem com custos baixos o mantém facilmente reciclável.")
        elif opcao_menu_info == 4:
          print("A reciclagem do Vidro é um processo aonde o vidro é reaproveitado, sendo utilizado para formação de novos materiais, isso acontece através do derretimento do vidro. A separação do vidro em cores pode ser necessária, dependendo da sua utilização final. As três cores principais são Verde, Marrom ou Âmbar e o Vidro Incolor.")
        elif opcao_menu_info == 5:
          print("Resíduo de computadores, conhecido como Lixo Eletrônico, tem seu nome por serem equipamentos eletrônicos descartados. Eles constituem um risco grande para o meio ambiente, por conterem metais tóxicos ou substâncias nocivas como Mercúrio, Chumbo, Berílio e Bário, entre outros.")
  ...
  if opcao_menu_principal == 3:
    ...
    while (opcao_menu_principal != 6):
        ...
        if opcao_menu_posicao == 1:
            print(Raio(float(input('Latitude: ')), float(input('Logitude: ')), float(input('Raio: ')), 'plástico'))

        elif opcao_menu_posicao == 2:
            print(Raio(float(input('Latitude: ')), float(input('Logitude: ')), float(input('Raio: ')), 'papel/papelão'))

        elif opcao_menu_posicao == 3:
            print(Raio(float(input('Latitude: ')), float(input('Logitude: ')), float(input('Raio: ')), 'metal'))

        elif opcao_menu_posicao == 4:
            print(Raio(float(input('Latitude: ')), float(input('Logitude: ')), float(input('Raio: ')), 'vidro'))

        elif opcao_menu_posicao == 5:
            print(Raio(float(input('Latitude: ')), float(input('Logitude: ')), float(input('Raio: ')), 'lixo eletronico'))
